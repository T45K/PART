...
internal class GitLogCommandTest {
    ...
    fun testLogCommand() {
        ...
        val result: List<Pair<ObjectId, String>> = logCommand.execute("src/org/DogManager#public_void_barkBark(Dog).mjava")

        assertEquals(5, result.size)
        assertEquals("<OriginalCommitID:d4bce13> Move Method barkBark from Dog to DogManager", result[0].second)
        assertEquals("<OriginalCommitID:a5a7f85> No refactoring.", result[1].second)
        assertEquals("<OriginalCommitID:0bb0526> Rename Operation bark(): void to barkBark(): void in class\n" +
                "org.animals.Dog.", result[2].second)
        assertEquals("<OriginalCommitID:40950c3> Extract Operation\tpublic takeABreath() : void extracted from public\n" +
                "bark() : void in class org.animals.Dog", result[3].second)
        assertEquals("<OriginalCommitID:cd61fd2> Versão inicial.", result[4].second)
    }
}